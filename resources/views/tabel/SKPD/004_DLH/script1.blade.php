<script>
    function sumKelurahans() {
        var kelurahanElements = document.querySelectorAll('[id^="kelurahan"]');
        var kelurahanSum = 0;
        for (var i = 0; i < kelurahanElements.length; i++) {
            if (kelurahanElements[i].id !== "kelurahant") {
                var kelurahanValue = parseFloat(kelurahanElements[i].value) || 0;
                kelurahanSum += kelurahanValue;
            }
        }
        document.getElementById('kelurahant').value = kelurahanSum;
    }

    var kelurahanInputs = document.querySelectorAll('[id^="kelurahan"]');
    for (var i = 0; i < kelurahanInputs.length; i++) {
        kelurahanInputs[i].addEventListener('focusout', sumKelurahans);
    }
</script>
<script>
    function sumDesas() {
        var desaElements = document.querySelectorAll('[id^="desa"]');
        var desaSum = 0;
        for (var i = 0; i < desaElements.length; i++) {
            if (desaElements[i].id !== "desat") {
                var desaValue = parseFloat(desaElements[i].value) || 0;
                desaSum += desaValue;
            }
        }
        document.getElementById('desat').value = desaSum;
    }

    var desaInputs = document.querySelectorAll('[id^="desa"]');
    for (var i = 0; i < desaInputs.length; i++) {
        desaInputs[i].addEventListener('focusout', sumDesas);
    }
</script>
<script>
    function sumDusuns() {
